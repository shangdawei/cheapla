...
static int response_la_process_request(struct http_state *http, const char *method, const char *url)
{
	if (strcmp(method, "GET"))
		return 0;
	...
	http->response_priv = mem_malloc(sizeof(struct response_la_priv_s));
	if (!http->response_priv)
		return 0;
...
static void response_la_finish(struct http_state *http)
{
	struct response_la_priv_s *priv = http->response_priv;
	mem_free(priv);
}
...
#ifndef UNIX
int response_flash_put_process_request(struct http_state *http, const char *method, const char *url);
void response_flash_put_process_header(struct http_state *http, const char *option, const char *val);
void response_flash_put_start_response(struct http_state *http);
int response_flash_put_do_header(struct http_state *http);
int response_flash_put_do_data(struct http_state *http);
int response_flash_put_process_data(struct http_state *http, const void *data, int len);
void response_flash_put_finish(struct http_state *http);
#endif
...
#endif
		{response_la_process_request, 0, 0, response_la_do_header, response_la_do_data, 0, response_la_finish},
